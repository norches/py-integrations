...
- Запись разговора прикрепляется в чат и записывается в поле обращения по готовности.
...
| `asterisk_recording_link_field_key` | Нет | String | `Ключ поля ссылки на запись` / `Yozuv havolasi maydon kaliti` / `Recording link field key` | `Ключ кастомного поля обращения, куда записывается ссылка на запись разговора` / `Qo'ng'iroq yozuvi havolasi yoziladigan murojaat maydoni kaliti` / `Custom ticket field key where the recording link is written` | `field_recording_link` / `field_recording_link` / `field_recording_link` |
...
| `asterisk_post_status_messages` | Нет | Boolean | `Сообщения о статусах звонка в чат` / `Chatga qo'ng'iroq holati xabarlari` / `Post call-status messages to chat` | `Если включено, в чат обращения публикуются системные сообщения по этапам звонка (начат/принят/завершён). По умолчанию выключено — выполняются только действия (создание/назначение/закрытие) и прикрепление записи` / `Yoqilsa, murojaat chatiga qo'ng'iroq bosqichlari bo'yicha tizim xabarlari yuboriladi. Standart o'chirilgan — faqat amallar (yaratish/biriktirish/yopish) va yozuv bajariladi` / `When enabled, per-stage system messages (started/answered/completed) are posted to the case chat. Off by default — only the actions (create/assign/close) and the recording are applied` | `false` / `false` / `false` |
